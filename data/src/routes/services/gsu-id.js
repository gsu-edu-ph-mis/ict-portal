...
router.post('/services/gsu-id', async (req, res, next) => {
    try {
        ...
        let refNum = `${passwordMan.genPasscode(4)}`
        let conflict = null;
        do {
            conflict = await req.app.locals.db.models.Gsuid.findOne({
                where: {
                    uid: refNum
                }
            });
            ...
            if (conflict) {
                // generate a new refNum if conflict exists
                refNum = `${passwordMan.genPasscode(4)}`
            }
        } while (conflict);
